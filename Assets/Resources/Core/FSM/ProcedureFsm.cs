...
public class ProcedureFsm
{
	...
	public void Run()
	{
		if (_nodeNames.Count > 0)
			...
			Debug.LogWarning("Procedure system dont has any node.");
	}
	...
	public void SwitchNext()
	{
		...
		{
			Debug.LogWarning($"Current node {_fsm.CurrentNodeName} is end node.");
		}
		else
		{
	...
	public void SwitchLast()
	{
		...
		if (index <= 0)
		{
			Debug.LogWarning($"Current node {_fsm.CurrentNodeName} is begin node.");
		}
		else
		{
			Switch(_nodeNames[index - 1]);
