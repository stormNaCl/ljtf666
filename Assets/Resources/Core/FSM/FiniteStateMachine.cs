...
public class FiniteStateMachine
{
	...
	public void AddNode(IFsmNode node)
	{
		...
		{
			Debug.LogWarning($"Node {node.Name} already existed");
		}
	}
	...
	public void Transition(string nodeName)
	{
		...
		if (node == null)
		{
			Debug.LogError($"Can not found node {nodeName}");
			return;
		}
		...
		if (Graph != null)
		{
			...
			{
				Debug.LogError($"Can not transition {_curNode} to {node}");
				return;
			}
		}

		Debug.Log($"FSM transition {_curNode.Name} to {node.Name}");
		_preNode = _curNode;
		_curNode.OnExit();
		_curNode = node;
		_curNode.OnEnter();
